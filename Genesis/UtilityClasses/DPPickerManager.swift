...
open class DPPickerManager: NSObject, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @objc open func showPickerWithTime(title: String?, selected: Date?, min: Date?, max: Date?, completion:DPPickerDateCompletion?) {
        self.showPickerWithTime(title: title, picker: { (picker) in
            picker.date = selected ?? Date()
            picker.minimumDate = min
            picker.maximumDate = max
            picker.timeZone = self.timeZone
    ...
    @objc open func showPickerWithTime(title: String?, picker:((_ picker: UIDatePicker) -> Void)?, completion:DPPickerDateCompletion?) {
    ...
    @objc open func showPicker(title: String?, view: UIView, completion:DPPickerCompletion?) {
        ...
        let image = UIImage(named: "closeOrange")?.withRenderingMode(.alwaysTemplate)
